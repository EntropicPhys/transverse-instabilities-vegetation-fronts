...
pde=stanpdeo1D(lx,2*lx/nx); p.pdeo=pde; p.vol=2*lx;p.np=pde.grid.nPoints; p.nu=p.nc.neq*p.np; p.sol.xi=1/(p.nu);p=setfemops(p); 
...
screenlayout(p); p.plot.bpcmp=2; p.plot.cl={'black','blue','red'}; plotsol(p); % generic plot parameters; see stanparam for details.
p.nc.ilam=1; p.nc.lammin=0; p.nc.neq=3; p.sw.sfem=-1; % continuation and discretization parameters; see stanparam for details 
