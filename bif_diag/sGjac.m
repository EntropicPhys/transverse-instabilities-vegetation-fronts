...
% sGjac calculates the Jacobian matrix of the discrete approximation made in sG.m
% Parameters extraction from u-field
par=u(p.nu+1:end); pp=par(1); Lam=par(2); nw=par(3); nh=par(4); 
ee=par(5); Rw=par(6); Rh=par(7); Ga=par(8); A=par(9); Q=par(10); 
ff=par(11); k=par(12);m=par(13);dw=par(14);dh=par(15);
% Definition of physical fields and others.
n=p.np;b=u(1:n);w=u(n+1:2*n);h=u(2*n+1:3*n); ov=ones(n,1);
% nodal-Jacobian entries  
f1b = -m*ov - Lam*w.*(1+ee*b).*(4*ee*b.^2-k+b*(2-3*ee*k))/k;
f1w = Lam*(1-b/k).*b.*(1+ee*b).^2;f1h = 0*ov;
