...
I=a*(b+q*ff)./(b+q); dI = a*q*(1-ff)./(b+q).^2;
Lw=nw./(1+Rw*b); dLw=-nw*Rw./(1+Rw*b).^2;
Lh=nh./(1+Rh*b); dLh=-nh*Rh./(1+Rh*b).^2;
Gw=ga*b.*(1+et*b).^2; dGw=ga*(1+et*b).*(1+3*et*b);
f1b=-1+w.*(1+3*et*(-2+et).*b.^2-4*(et^2)*b.^3+(4*et-2)*b);
f1w=(1-b).*b.*(1+et*b).^2;f1h=zeros(p.np,1);
f2b=dI.*h-dLw.*w-dGw.*w; f2w=-Gw-Lw; f2h=I;
f3b=-(dLh+dI).*h;f3w=zeros(p.np,1);f3h=-I-Lh;
